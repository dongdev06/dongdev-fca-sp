            ...
            logger('Cập Nhật Đã Lỗi Tiến Hành Xóa Package', "《 FCA - SP 》");
                execSync('npm cache clean --force', { stdio: 'ignore'})
                    await new Promise(resolve => setTimeout(resolve, 2*1000))
                        fs.removeSync('../fca-dongdev-remake');
                            // why stdio is not studio :v 
                        await new Promise(resolve => setTimeout(resolve, 2*1000))
                    execSync('npm i fca-dongdev-remake@latest', { stdio: 'ignore'})
                logger("Đã Thành Công - Tiến Hành Restart", "《 FCA - SP 》");
            process.exit(1);
        }
        catch (e) {
            logger("Đã Bị Lỗi Hãy Nhập Vào Console Mã Sau Đây Để Fix !", "《 FCA - SP 》");
                logger("rmdir ./node_modules/fca-dongdev-remake && npm i fca-dongdev-remake@latest && npm start", '[ FCA - SP ]');
